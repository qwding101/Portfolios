...
df_12dim = df[,c(1:5,12:21,10,11)]
df_12dim[,6:ncol(df_12dim)] = rowScale(df_12dim[,6:ncol(df_12dim)], center = T, scale = T, add_attr = F)
df_12dim[which(df_12dim$q2to4Mean == "NaN"), 6:ncol(df_12dim)] = 0 



colnames(df_6dim) = c("ID", "學制", "國中或高中樣本", "性別", "最高學歷", "領導觀", "課程觀", "教學觀", "評鑑觀", "變革喜好", "變革願意實踐")
colnames(df_12dim) = c("ID", "學制", "國中或高中樣本", "性別", "最高學歷", "分享決定", "社群共學", "學校自主", "統整應用", "引導教學", "體驗教學", "合作學習", "情境學習", "評鑑效用", "評鑑執行", "變革喜好", "變革願意實踐")
...
df_12dim_j = subset(df_12dim, `國中或高中樣本` == 0)

E.dist.6j2 = dist(df_6dim_j[,6:11], method = "euclidean")^2
E.dist.12j2 = dist(df_12dim_j[,6:17], method = "euclidean")^2

set.seed(20190903)
h.E.w.cluster.6j = hclust(E.dist.6j2, method = "ward.D2")
h.E.w.cluster.12j = hclust(E.dist.12j2, method = "ward.D2")
...
tree.12jstd = fviz_dend(h.E.w.cluster.12j, show_labels = F,#cex = 0.5,
                   k = 3, k_colors = c("#00AFBB", "#E7B800", "#FC4E07"), #"darkorchid"
                   color_labels_by_k = T,
                   rect = T) + 
  theme(legend.position = "bottom",
...
saveRDS(tree.12jstd, file = "tree12jstd.RDS")



fviz_nbclust(df_6dim_j[,6:11], # df_12dim[,6:17]
             FUNcluster = kmeans,
             method = "wss",     # total within cluster sum of square
             k.max = 7         # max number of clusters to consider
             ) +
...
fviz_nbclust(df_12dim_j[,6:17], # df_12dim[,6:17]
             FUNcluster = kmeans,
             method = "wss",     # total within cluster sum of square
             k.max = 7          # max number of clusters to consider
             ) +
...
kmeans.cluster3.12j = kmeans(df_12dim_j[,6:17], centers = 3, nstart = 100)
...
PCA.3grp.12j = fviz_cluster(kmeans.cluster3.12j,  
             data = df_12dim_j[,6:17],             
             geom = c("point"), #c("point","text") shows both point & label
             ellipse.type = "norm",
             star.plot = T # Add segments from centroids to items
             ) +
...
kmeans.cluster3.12j$size

# Combine the cluster group info into the original data frame
df_6dim_j$Cluster_3grp = factor(kmeans.cluster3.6j$cluster)
df_12dim_j$Cluster_3grp = factor(kmeans.cluster3.12j$cluster)

stat.3gp.6j = describeBy(df_6dim_j, group = df_6dim_j$Cluster_3grp, digits = 4) 
stat.3gp.12j = describeBy(df_12dim_j, group = df_12dim_j$Cluster_3grp, digits = 4) 

stat.3gp.df.6j = dfwithMSD(stat.3gp.6j)
stat.3gp.df.12j = dfwithMSD12(stat.3gp.12j)

profile.3gp.6j = CAprofile(stat.3gp.df.6j) + scale_x_discrete(limits=c("領導觀", "課程觀", "教學觀", "評鑑觀", "變革喜好", "變革願意實踐"))
profile.3gp.12j = CAprofile(stat.3gp.df.12j) + scale_x_discrete(limits=c("分享決定", "社群共學", "學校自主", "統整應用", "引導教學", "體驗教學", "合作學習", "情境學習", "評鑑效用", "評鑑執行", "變革喜好", "變革願意實踐"))
...
colorlow <- hcl.colors(3, alpha = .5)[kmeans.cluster3.12j$cluster]
pairs(df_12dim_j[,6:17], pch = 16, gap = 0, col = colorlow, oma=c(3,3,3,15))
par(xpd = T) # allow plotting of the legend outside the figure region  (ie within the space left by making the margins big)
legend(0.75, 0.675, legend = 1:3,  fill = hcl.colors(3), bty = "n", title = "Cluster")
```
...
df_12dim_s = subset(df_12dim, `國中或高中樣本` == 1)

E.dist.6s2 = dist(df_6dim_s[,6:11], method = "euclidean")^2
E.dist.12s2 = dist(df_12dim_s[,6:17], method = "euclidean")^2

set.seed(20190903)
h.E.w.cluster.6s = hclust(E.dist.6s2, method = "ward.D2")
h.E.w.cluster.12s = hclust(E.dist.12s2, method = "ward.D2")
...
tree.12sstd = fviz_dend(h.E.w.cluster.12s, show_labels = F,#cex = 0.5,
                   k = 3, k_colors = c("#00AFBB", "#E7B800", "#FC4E07"), #"darkorchid"
                   color_labels_by_k = T,
                   rect = T) + 
  theme(legend.position = "bottom",
...
saveRDS(tree.12sstd, file = "tree12sstd.RDS")
...
fviz_nbclust(df_12dim_s[,6:17], # df_12dim[,6:17]
             FUNcluster = kmeans,
             method = "wss",     # total within cluster sum of square
             k.max = 7          # max number of clusters to consider
             ) +
...
kmeans.cluster3.12s = kmeans(df_12dim_s[,6:17], centers = 3, nstart = 100)
...
PCA.3grp.12s = fviz_cluster(kmeans.cluster3.12s,  
             data = df_12dim_s[,6:17],             
             geom = c("point"), #c("point","text") shows both point & label
             ellipse.type = "norm",
             star.plot = T # Add segments from centroids to items
             ) +
...
kmeans.cluster3.12s$size

# Combine the cluster group info into the original data frame
df_6dim_s$Cluster_3grp = factor(kmeans.cluster3.6s$cluster)
df_12dim_s$Cluster_3grp = factor(kmeans.cluster3.12s$cluster)

stat.3gp.6s = describeBy(df_6dim_s, group = df_6dim_s$Cluster_3grp, digits = 4) 
stat.3gp.12s = describeBy(df_12dim_s, group = df_12dim_s$Cluster_3grp, digits = 4) 

stat.3gp.df.6s = dfwithMSD(stat.3gp.6s)
stat.3gp.df.12s = dfwithMSD12(stat.3gp.12s)
 
profile.3gp.6s = CAprofile(stat.3gp.df.6s) + scale_x_discrete(limits=c("領導觀", "課程觀", "教學觀", "評鑑觀", "變革喜好", "變革願意實踐"))
profile.3gp.12s = CAprofile(stat.3gp.df.12s) + scale_x_discrete(limits=c("分享決定", "社群共學", "學校自主", "統整應用", "引導教學", "體驗教學", "合作學習", "情境學習", "評鑑效用", "評鑑執行", "變革喜好", "變革願意實踐"))
...
colorlow <- hcl.colors(3, alpha = .5)[kmeans.cluster3.12s$cluster]
pairs(df_12dim_s[,6:17], pch = 16, gap = 0, col = colorlow, oma=c(3,3,3,15))
par(xpd = T) # allow plotting of the legend outside the figure region  (ie within the space left by making the margins big)
legend(0.75, 0.675, legend = 1:3,  fill = hcl.colors(3), bty = "n", title = "Cluster")
```